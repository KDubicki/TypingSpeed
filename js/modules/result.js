...
export const show = ({ total, correct, time } = {}) => {
    ...
    results.innerHTML = `
          <p class="modal__result">Speed: ${ Math.round((total / time) * 100) / 100 } chars / s</p>
          <p class="modal__result">Accuracy: ${ Math.round((correct / total) * 100)}%</p>
          <p class="modal__result">Correct chars: ${ correct }</p>
          <p class="modal__result">Incorrect chars: ${ incorrect }</p>
          <p class="modal__result">Total chars: ${ total }</p>
